...
export default function RightMenu({onButtonClick}: Props) {
  ...
  return (
    <div className="pb-[13px] rounded-[5px] w-[340px] h-full bg-gradient-to-b from-[#93A1F8] to-[#42D2C9] flex flex-col items-center">
      ...
      <Input
        ...
        onChange={(e) =>
          setFill(
            isValueIntOrFloat(e.target.value) ? +e.target.value : fill
          )
        }
      />
      <div className="mt-[15px] flex items-center gap-[1px]">
        <ButtonGroup>
            {fillButtons.map((button) => (
                <Button
                onClick={() => setFill(Math.floor((userDeposit.data ?? 0) / 10))}
                size="sm"
                key={button.value}
